...
public class MainActivity extends AppCompatActivity {
    ...
    SeekBar seekBarForceTheshold; // aktuell nicht verwendet
    TextView menubutton;
    ...
    private void setupChart() {
        ...
        actualDataSet = new LineDataSet(new ArrayList<>(), "Ist-Wert");
        ...
        forceDataSet = new LineDataSet(new ArrayList<>(), "Kraft-Ist-Wert");
    ...
    private void setupThresholdEdit() {
    ...
    private void handleThresholdInput(String input) {
        ...
        try {
            ...
            forceThreshold.setText(String.format("%.2f N", newtonValue));
            startActivity(intent);
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if ("ACTION_DATA_AVAILABLE".equals(intent.getAction())) {
                ...
                float currentTime = intent.getIntExtra("currentTime", 0) / 10.0f;
                ...
                force.setText(String.format("%.2f N", forcevalue / 1000.0f));

                if (profileStarted) {
                    actualDataSet.addEntry(new Entry(currentTime, (float) temp));
                    forceDataSet.addEntry(new Entry(currentTime, forcevalue / 1000.0f));
                }
    ...
    // ---- Touch für Sollprofil ------------------------------------------------

    private void setupTouchListener() {
        lineChart.setOnTouchListener((v, event) -> {
            float touchX = event.getX();
            float touchY = event.getY();
            MPPointD point = lineChart.getTransformer(YAxis.AxisDependency.LEFT)
                    .getValuesByTouchPoint(touchX, touchY);
